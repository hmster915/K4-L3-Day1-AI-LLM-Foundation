...
def compare_models(prompt: str) -> dict:
    ...
    # 2. Estimate the cost based on the formula provided in the hint
    cost = (len(gpt4o_text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    
    # 3. Return the exact dictionary the test expects
    return {
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    # raise NotImplementedError("Implement estimate_cost")
        # 1. Count tokens for both input and output
    input_tokens = count_tokens(prompt, model)
    output_tokens = count_tokens(response, model)
    
    # 2. Get pricing (with fallback to gpt-4o pricing for unknown models)
    pricing = PRICING_PER_1K_TOKENS.get(model, PRICING_PER_1K_TOKENS["gpt-4o"])
    
    # 3. Calculate costs (Pricing is per 1000 tokens)
    input_cost = (input_tokens / 1000) * pricing["input"]
    output_cost = (output_tokens / 1000) * pricing["output"]
    total_cost = input_cost + output_cost
    
    # 4. Return the exact dictionary expected by the tests
    return {
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Read the stream chunk by chunk and print it immediately
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
            reply += delta
        print() # Add a newline when the bot is done talking
        
        # Save the conversation to history and trim it to the last 3 turns (6 messages)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt == max_retries:
                raise  # If we run out of retries, throw the error
            
            # Wait for base_delay * (2 ^ attempt) seconds before trying again
...
def run_assistant(
    ...
    while True:
        # Check max turns BEFORE asking for input
        if max_turns is not None and num_turns >= max_turns:
            break
        ...
        # 3. Combine Persona + History + New Message
        messages = [{"role": "system", "content": persona}] + history + [{"role": "user", "content": user_msg}]
        
        # 4. Call API using your retry logic and streaming
        ...
        # 5. Read the stream and print it
        reply = ""
        for chunk in stream:
            delta = chunk.choices[0].delta.content or ""
            print(delta, end="", flush=True)
        ...
        # 6. Save history and trim it to the last 3 turns (6 messages)
        history.append({"role": "user", "content": user_msg})
        history.append({"role": "assistant", "content": reply})
        history = history[-6:]
